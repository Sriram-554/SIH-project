...
st.markdown("""
<style>
    .main-title { font-size: 2.1rem; font-weight: 800; background: linear-gradient(90deg, #00d2ff 0%, #3a7bd5 50%, #00f2fe 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 0.1rem; }
    .sub-title { font-size: 0.95rem; color: #a0aec0; margin-bottom: 1.2rem; }
    .metric-box { background-color: #111827; border: 1px solid #1f2937; border-radius: 8px; padding: 14px; margin-bottom: 10px; }
    .trace-card { background-color: #0d1117; border: 1px solid #30363d; border-radius: 6px; padding: 12px; font-family: monospace; font-size: 0.88rem; }
    .badge { display: inline-block; padding: 2px 8px; border-radius: 4px; font-size: 0.78rem; font-weight: 600; margin-right: 4px; }
...
with tab_assistant:
    ...
    with col_prompt:
        ...
        with st.form("agent_query_form"):
            user_input = st.text_input("Enter your satellite query:", value=st.session_state.chosen_query, placeholder="e.g. Has the built-up area increased, decreased, or remained unchanged?")
            submit_btn = st.form_submit_button("🚀 Run Agentic SatQuery Pipeline", use_container_width=True)
    if submit_btn and user_input:
        with st.spinner("Agentic Controller routing query..."):
            agent_res = controller.execute_query(query=user_input, primary_input=primary_file if primary_file else "outputs/sentinel_rgb.png", secondary_input=secondary_file if secondary_file else None, modalities=input_modalities)
            ...
            with st.container():
                ...
                if item.get("visuals"):
                    ...
                    for idx, visual_path in enumerate(item["visuals"]):
                        with v_cols[idx % 2]:
                            st.image(visual_path, width='stretch', caption="Generated evidence visualization")
                st.markdown(f'<span class="badge badge-blue">Tool: {item["tool"]}</span><span class="badge badge-purple">Task: {item["task"]}</span><span class="badge badge-green">Confidence: {int(item["confidence"]*100)}%</span><span class="badge badge-blue">Trace ID: {item["trace_id"]}</span>', unsafe_allow_html=True)
                st.divider()
...
with tab_trace:
    ...
    with col_t1:
        ...
        if st.session_state.latest_trace:
            st.json(st.session_state.latest_trace)
        else:
            sample_trace = {
                ...
                "input_summary": {"image_count": 2, "modalities": ["optical", "sar"], "formats": [".png", ".png"], "compatibility_check": "PASSED", "validation_message": "Input configuration validated."},
                "parameters_configured": {"max_tokens": 350, "temperature": 0.2, "spectral_telemetry_injected": True, "domain_adaptation_applied": "Standard-RS"},
...
with tab_cross_modal:
    ...
    st.caption("Fuses optical multi-spectral reflectance (Sentinel-2 / Cartosat) with SAR radar backscatter (Sentinel-1 / RISAT). Results shown below are heuristic candidates, not validated physical classifications.")
    ...
    if st.button("⚡ Run Cross-Modal Optical-SAR Joint Fusion", use_container_width=True):
        try:
            ...
            m1.metric("Built-up Candidate", f"{fusion_res['urban_percentage']}%", help="Heuristic candidate estimate from the current optical/SAR fusion implementation; not a validated double-bounce classifier.")
            m2.metric("Water Candidate", f"{fusion_res['water_percentage']}%", help="Heuristic candidate estimate from low-intensity fusion response; not a validated water classifier.")
            m3.metric("Vegetation Canopy Estimate", f"{fusion_res['vegetation_percentage']}%", help="Heuristic vegetation estimate derived from the current fusion implementation.")
            m4.metric("Fusion Complementarity", "Qualitative", help="The current prototype combines optical and SAR information but does not calibrate SAR backscatter to physical dB for a validated threshold claim.")
...
with tab_spectral:
    ...
    with col_s2:
        ...
        clc_preds = controller.taxonomy.classify_from_spectral_metrics(ndvi_mean=0.412, water_pct=3.8, dense_veg_pct=42.1, mod_veg_pct=34.5, barren_pct=19.6, sar_backscatter_db=-8.5)
        for p in clc_preds:
...
with tab_change:
    ...
    cd_query = st.selectbox("Select Change Query for CDVQA:", ["Has the built-up area increased, decreased, or remained unchanged?", "What changed between these two dates, and where did the change occur?", "Assess vegetation gain and loss across the region."])
...
with tab_grounding:
    ...
    target_f = st.selectbox("Select Target to Locate & Bounding Box:", ["Water Body / River / Lake", "Dense Forest Canopy", "Vegetation & Cropland", "Urban / Built-up Cluster"])
    target_img = primary_file if (primary_file and Path(str(primary_file)).exists() and not Path(str(primary_file)).is_dir()) else (Path("outputs/sentinel_rgb.png") if Path("outputs/sentinel_rgb.png").exists() else Path("data/samples/sample_optical_t1.png"))
...
with tab_meta:
    ...
    with c_m2:
        st.markdown("**SAR Radar Sensor:** `Sentinel-1 C-SAR / RISAT-1 C-band SAR`")
        st.markdown("**Polarizations:** `VV (Vertical-Vertical), VH (Vertical-Horizontal)`")
        st.markdown("**Spatial Resolutions:** `10m, 20m, 60m`")
...
"""
    ...
    st.download_button("📥 Download SIH Evaluation Report (.md)", data=report_text, file_name="satquery_sih_evaluation_report.md", mime="text/markdown")
